...
class AnnotatorInterface:
    ...
    def set_video_label(self, video_path):
     self.current_video_label = Path(video_path).parent.name
    ...
    def _save_frame_data(self, persons):
            if not persons or self.current_video_label is None:
                return

            frame_keypoints = []
            for person in persons:
                joints2d = person['pose_2d'].get_joints()
                keypoints = [coord for joint in joints2d for coord in joint]
                frame_keypoints.extend(keypoints)

            if frame_keypoints:
                self.keypoints_file.write(' '.join(map(str, frame_keypoints)) + '\n')
                self.labels_file.write(str(self.current_video_label) + '\n')

    """
    Return a json containing the annotations
    """
    def jsonify(self, output_file='pose_predictions.json'):
        persons = self.get_persons()
        annotations = []
